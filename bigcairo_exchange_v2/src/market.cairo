...
use bigcairo_exchange_v2::order::{CancelOrder, TakeOrderInput, Order};
use bigcairo_exchange_v2::collection_offer::{TakeCollectionOffer, CollectionOffer};
use array::{Array};
#[starknet::interface]
trait Imarket<T> {
    // admin function
...
#[starknet::contract]
mod BigCairo {
    use bigcairo_exchange_v2::order::IStructHash;
    use bigcairo_exchange_v2::order::IOffchainMessageHash;
    use bigcairo_exchange_v2::collection_offer::IStructHashCollectionOfferr;
    use starknet::{
    ...
    use bigcairo_exchange_v2::order::{Order, OffchainMessageHashOrderStruct, StarknetDomain};
    use bigcairo_exchange_v2::order::{CancelOrder, TakeOrderInput};
    use bigcairo_exchange_v2::collection_offer::{
        TakeCollectionOffer, CollectionOffer, OffchainMessageHashCollectionOfferStruct
    };
    use bigcairo_exchange_v2::order::IStructHashOrder;
    ...
    struct Storage {
        ...
        amount_taken: LegacyMap<
            (ContractAddress, felt252), felt252
        >, // (userAddress, orderhash) => amount_taken
    ...
    impl ImpleMarket of super::Imarket<ContractState> {
        ...
        fn take_ask(ref self: ContractState, input: TakeOrderInput) {
            ...
            self.assert_erc20_balance_allowance(order.price, order.amount, caller, this_contract);
            ...
            self.transer_erc20(caller, fund_address, protocol_fee);
            if (royalty_fee != 0) {
                self.transer_erc20(caller, recipent_royalty_addr, royalty_fee);
            }
            self.transer_erc20(caller, trader_addr, seller_price);
    ...
    impl InternalFunctions of InternalFunctionsTrait {
        ...
        fn transer_erc20(
            ...
            let strk_addr = contract_address_try_from_felt252(
                0x04718f5a0fc34cc1af16a1cdee98ffb20c31f5cd61d6ab07201858f4287c938d
            )
                .unwrap();
            IERC20Dispatcher { contract_address: strk_addr }
        ...
        fn assert_erc20_balance_allowance(
            self: @ContractState,
            price_item: felt252,
            amount: felt252,
            owner: ContractAddress,
            ...
            let strk_addr = contract_address_try_from_felt252(
                ...
                .unwrap();
            let allowance = IERC20Dispatcher { contract_address: strk_addr }
                .allowance(owner, spender);
            let must_pay: u256 = price_item.into() * amount.into();
            assert(allowance >= must_pay, 'Invalid Approve');
        }
